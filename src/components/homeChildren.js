...
// import { View, Text, StyleSheet, AsyncStorage, Dimensions, TouchableOpacity, ImageBackground, FlatList, Alert } from 'react-native';
...
//             axios.get(`https://pcportal.malangis.com/public/api/childrentasks/${id}`)
//                 .then((response) => {
...
//                                 <Text style={{ fontSize: 20, marginLeft: 50, fontWeight: 'bold', color: '#fff' }}>Child DashBoard</Text>
//                             </View>
//                         </View>
//                     </View>
//                     <View style={{ height: '10%', backgroundColor: '#651fff', }}>
//                         <View style={{ marginTop: 20, }}>
//                             <Text style={{ fontSize: 15, alignSelf: 'center', color: '#fff', fontWeight: 'bold' }}>Assigned Tasks</Text>
//                         </View>
//                     </View>
//                     {/* <TouchableOpacity onPress={()=>{this.props.navigation.navigate('task_Detail_Children')}}  style={{ height: '9%', justifyContent: 'space-between', marginHorizontal: 25, borderRadius: 5, backgroundColor: '#651fff', marginTop: 15, flexDirection: 'row' }}>
...
export default class homeChildren extends Component {
    ...
    componentDidMount() {
        AsyncStorage.getItem('userId').then((id) => {
                axios.get(`https://pcportal.malangis.com/public/api/tasklist/${id}`)
                    .then((response) => {
                        console.log(response.data.tasklist)
                        if (response.data.success === "1") {
                            this.setState({ responseData: response.data.tasklist })
            ...
            renderRow = ({ item }) => {
                // console.log(item.englishName);
                return (
                    <TouchableOpacity onPress={() => { this.props.navigation.navigate('taskShowDetailChildren',{ChildrenTaskId:item.id }) }} 
                    style={{ height: 50, justifyContent: 'space-between', marginHorizontal: 25, borderRadius: 5, backgroundColor: '#651fff', marginTop: 15, flexDirection: 'row' }}>
                        <Text style={{ fontSize: 15, color: '#fff', marginLeft: 15, marginTop: 16, fontWeight: 'bold' }}>{item.title}</Text>
                        <Text style={{ fontSize: 15, color: '#fff', marginRight: 15, marginTop: 16, fontWeight: 'bold' }}>{item.task_points}</Text>
                    </TouchableOpacity >
