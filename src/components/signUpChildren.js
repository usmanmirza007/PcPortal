...
export default class signUpChildren extends Component {
  ...
  onSigUp(email, password, username) {
    ...
    if (this.state.email === '' || this.state.password === '' || this.state.username === '') {
      ...
      axios.post(`https://pcportal.malangis.com/public/api/children_register`,
        {
          'email': email,
          'password': password,
          'username': username,
      ...
      <View style={{ flex: 1 }}>
        <ImageBackground source={require('./../images/bg3.png')} style={{ width: '100%', height: '100%', }}>
          <View style={{ flex: 4, alignItems: 'center', marginTop: '5%', justifyContent: 'center' }}>
            ...
            <Text style={{ color: '#fff', fontSize: 20, fontWeight: 'bold', marginBottom: 60,  }}>Children SignUp</Text>
          </View>
...
*/}
            ...
            {this.renderButton()}
              <Text style={{ color: '#fff', fontSize: 17, marginTop: 10}}>Already have account? <Text style = {{fontSize: 18, fontWeight: 'bold',}} onPress={() => { this.props.navigation.navigate('childrenLogin') }}>Login</Text></Text>
          </View>
        </ImageBackground>
      </View>
    );
